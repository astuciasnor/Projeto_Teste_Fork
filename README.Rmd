...
-   **Explicação do merge:** Se você já vinha trabalhando em uma branch, é crucial incorporar as últimas alterações da `master` nela. Isso pode ser feito via `git merge master` ou `git rebase master`. O `merge` cria um novo commit de mesclagem, enquanto o `rebase` reaplica seus commits sobre as mudanças da `master`.
-   **Atenção**: no mometo de fazer Pull Request, você deve ir na branch do Github correspondente aquela que ainda está usando a fim de poder visualizar o botão de solicitação de Pull Request
...
1.  Acesse o seu repositório no GitHub (Pode ser preciso ter que ir na branch de trabalho certa)
